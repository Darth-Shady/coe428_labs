...
static int top = -1;  // changed to -1 since it's more intuitive that way. equal to the number of indices, not the number of elements
...
int isEmpty()
{
  ...
  return top == -1;  //boolean return, check if stack empty, 0  if false, non zero if true
}

char *  pop()
{
  if (isEmpty()) { //reusing dunctions
    fprintf(stderr, "Error: empty stack\n");
    return NULL;
  } else {

  char * result = stack[top]; // needs to return this to validate XML tags
  top--;
  return result; }
}

//used top as the index of the last element (initialized to -1) because it makes the push/pop operations more intuitive
